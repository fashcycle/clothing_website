...
export default function ProductPage({ id }: { id: string }) {
  ...
  useEffect(() => {
    if (product) {
      const images = Object.entries(product?.productImage || {})
        .filter(
          ([key, path]) =>
            path &&
        ...
        >
                ...
                <button
                  ...
                  onClick={() => {
                    const currentIndex = productImages?.findIndex(
                      (img: any) => img.path === selectedImage
                    );
                    const prevIndex =
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            {product?.proofOfPurchase && (
              <div
                className={`relative aspect-square rounded-md overflow-hidden cursor-pointer border ${
                  selectedImage === product?.proofOfPurchase
                    ? "border-black"
                    : "border-gray-200"
                }`}
        ...
        >
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-700">
              {product?.description ||
                `Quality ${product?.productName} in ${product?.color}. Size ${product?.size} with ${product?.sizeFlexibility} flexibility.`}
            </p>
          </div>
            ...
            <div className="flex justify-between items-center py-2 border-b border-gray-100">
              ...
              product.listingType.includes("rent") ? (
                <span className="font-medium">
                  ₹{Math.round((product?.originalPurchasePrice * 21) / 100)} for
                  3 days
                </span>
              ) : (
                <span className="font-medium">
